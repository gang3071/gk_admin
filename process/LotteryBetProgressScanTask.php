...
class LotteryBetProgressScanTask
{
        ...
        try {
            ...
            foreach ($activities as $activity) {
                try {
                    ...
                    // Step 1: ✅ 使用覆盖方式重新计算（核心方法）
                    $recalculated = $this->recalculateBetProgress($activity->id, $playerBetAmounts);
                    $totalRecalculated += $recalculated;

                    // Step 2: ⭐ 检查并发券（关键！）
                    $ticketsIssued = $this->checkAndIssueTicketsForFullScan($activity->id, array_keys($playerBetAmounts));
     ...
     * - 查询所有达标玩家并发放摸奖券
     * - 使用乐观锁防止重复发券
     * - 来源设置为 SOURCE_BETTING（打码获得）
     *
     * @param int $activityId 活动ID
     * @param array $playerIds 玩家ID列表
     * @return int 发放的券数
     */
    protected function checkAndIssueTicketsForFullScan(int $activityId, array $playerIds): int
    {
        if (empty($playerIds)) {
            return 0;
        }
                ...
                try {
                    ...
                    try {
                        ...
                        $result = $issueService->issueTicketsBatch(
                            ...
                            $progress->player_id,
                            $ticketsToIssue,
                            LotteryTicket::SOURCE_BETTING  // ✅ 打码来源
                        );
