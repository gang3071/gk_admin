...
⚠️ Database Field Deletion Process: When deleting database fields, follow the strict process in DATABASE_FIELD_DELETION_GUIDE.md to avoid production bugs.
...
C. Background Processes (20+ custom processes): Defined in config/process.php:

Settlement: ProfitSettlement, ChannelSettlement, NationalPromoterRebate, ReverseWater
Machine Management: MachineKeepOutPlayer, SyncMachineActivity, ClearAbnormalMachine
Real-time Communication: LotteryPoolSocket, OnlinePlayerSocket, GamePoolSocket
Cleanup: LogClear, MediaRecordingClear, MediaClear, BurstCleaner
Notifications: LotteryRemind, RechargeRemind, WithdrawRemind
Streaming: TencentStream, GetAmsViewers, GetTencentViewers
Inter-Project Communication
Key Communication Patterns:

Game Platform Proxy (gk_admin/gk_api → gk_work):
...
WebSocket Push (gk_admin/gk_work → gk_api):
...
Department/Channel (部门/渠道) - Top level organizational unit
Agent (代理) - Mid-level, can have multiple stores
Store (店家) - Bottom level, directly manages players
Each tier has:
...
📖 Complete ExAdmin Guide: EXADMIN_FRAMEWORK_GUIDE.md
...
File-Based Configuration - Permissions are defined in PHP configuration files
Changes take effect after server restart - php start.php restart
NO database synchronization needed - Configuration files are version-controlled
...
Menu Node Configuration (菜单节点配置)
...
Menu Naming Conventions (菜单命名规范)
...
common.php - Shared translations across modules
...
{module}.php - Module-specific translations
...
label - UI labels with colon suffix (for detail views)
...
Identify the module - Determine which translation file to use

Shared/common text → common.php
Module-specific → {module}.php (e.g., player.php, shift_handover.php)
Choose the category - Use standard categories (fields, label, action, filter, status, error)

Create the key - Use descriptive, dot-notation keys

Add to ALL 4 language files - Start with zh-TW (Traditional Chinese) as default, then add zh-CN, en, jp
...
NEVER hardcode user-facing text - Always use admin_trans()
Default to Traditional Chinese (zh-TW) - Write zh-TW translations first, then translate to other languages
Add to all 4 language files - zh-TW (first), zh-CN, en, jp
Use structured keys - Follow {module}.{category}.{name} pattern
Group related translations - Use nested arrays for logical grouping
Support parameters - Use {param} placeholders for dynamic content
Be descriptive - Key names should be self-documenting
Test language switching - Verify all languages display correctly
Keep translations synchronized - When modifying any language file, update all 4 languages
Reference Files:
...
Routes - Auto-discovered by ExAdmin, no manual registration

Modifying Existing Features
Read the controller to understand current logic
Check service layer in addons/webman/service/ for business logic
Update translations if changing user-facing text
Test across all user types (channel/agent/store admin)
...
❌ Don't use player.machine relationship in offline channel controllers
❌ Don't display player.phone in store/agent backends
❌ Don't use "player" terminology in UI - use "device" (设备)
❌ Don't mix online and offline patterns in the same controller
❌ Don't forget to check channel.is_offline before applying this pattern
...
No phone numbers or personal player info -店家 (store owner) manages devices, not people
...
Permission denied: Check admin_permission and user role
Data not showing: Check DataPermissions and department_id
Translation missing: Add to all 4 language files
Form not saving: Check saving() hook for errors
...
Critical Dependencies: The project requires all Composer dependencies to be properly installed. Missing packages will cause runtime errors.
...
Webman Workers: Edit start.php to adjust worker count:
...
用户权限模块 - 三层权限架构（渠道/代理/店家）、5级数据权限、RBAC
玩家管理模块 - 25+模型、三种玩家类型、推广员体系、多钱包系统
机台管理模块 - 20+模型、三种机台类型（斯洛/钢珠/捕鱼）、实时工控通信
游戏平台集成模块 - 20+游戏平台、转账钱包/单一钱包双模式
财务管理模块 - 充值/提现/转点、多平台钱包、财务记录
营销活动模块 - 活动系统、存款优惠、公告、轮播图
分润结算模块 - 推广员分润、渠道分润、全民代理返佣
彩金系统模块 - 彩金池配置、机台彩金、游戏彩金、保底机制
数据统计报表模块 - 机台报表、玩家报表、财务报表
其他核心模块 - 自动交班、设备管理、系统配置、内容管理
📖 详细模块分析文档： SYSTEM_MODULES.md
