...
class PlayerDeliveryRecordController
{
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model(), function (Grid $grid) {
            ...
            $grid->model()->with(['player', 'machine', 'player.recommend_promoter'])->orderBy('created_at', 'desc');
                ...
                ->display(function ($value) {
                    switch ($value) {
                        ...
                        case PlayerDeliveryRecord::TYPE_REVERSE_WATER_POOL:
                            ...
                            break;
                        case PlayerDeliveryRecord::COIN_ADD:
                            $tag = Tag::create(admin_trans('player_delivery_record.type.' . PlayerDeliveryRecord::COIN_ADD))->color('#e8c521');
                            break;
                        case PlayerDeliveryRecord::COIN_DEDUCT:
                            $tag = Tag::create(admin_trans('player_delivery_record.type.' . PlayerDeliveryRecord::COIN_DEDUCT))->color('#e8c521');
                            break;
                        case PlayerDeliveryRecord::TYPE_SPECIAL:
            ...
            $grid->column('amount', admin_trans('player_delivery_record.fields.amount'))->display(function (
                ...
                switch ($data->type) {
                    ...
                    case PlayerDeliveryRecord::TYPE_SPECIAL:
                        // 特殊类型：根据 amount_before 和 amount_after 判断是增加还是扣除
            ...
            $grid->filter(function (Filter $filter) {
                ...
                $filter->like()->text('player.recommend_player.uuid')->placeholder(admin_trans('promoter_profit_record.player_promoter.uuid'));
    ...
    public function detail($machine): Detail
    {
        return Detail::create($machine, function (Detail $detail) {
            ...
            $detail->item('type', admin_trans('game_type.fields.type'))->display(function ($val) {
                $color = 'orange';
    ...
    {
        $department_id = Request::input('data.department_id');
        ...
        $list = Activity::query()->with(['activity_content' => function ($query) use ($lang) {
            ...
            $query->whereJsonContains('department_id', $department_id);
        })->get()->toArray();
        $data = [];
