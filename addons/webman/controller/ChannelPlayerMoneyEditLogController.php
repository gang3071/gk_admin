 ...
 * @group channel
 */
class ChannelPlayerMoneyEditLogController
{
    protected $model;
        ...
        return Grid::create(new $this->model, function (Grid $grid) {
            ...
            $totalData = $query->selectRaw(
                'sum(IF(action = 0, money, 0)) as total_recharge,
                        ...
                        sum(IF(action = 3, money, 0)) as total_other, sum(IF(action = 4, money, 0)) as total_activity_give, 
            ...
            $layout->row(function (Row $row) use ($totalData) {
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_activity_give'))->value(!empty($totalData['total_activity_give']) ? floatval($totalData['total_activity_give']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_triple_seven_give'))->value(!empty($totalData['total_triple_seven_give']) ? floatval($totalData['total_triple_seven_give']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_composite_machine_give'))->value(!empty($totalData['total_composite_machine_give']) ? floatval($totalData['total_composite_machine_give']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_real_person_give'))->value(!empty($totalData['total_real_person_give']) ? floatval($totalData['total_real_person_give']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_electronic_give'))->value(!empty($totalData['total_electronic_give']) ? floatval($totalData['total_electronic_give']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_other'))->value(!empty($totalData['total_other']) ? floatval($totalData['total_other']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_admin_deduct'))->value(!empty($totalData['total_admin_deduct']) ? floatval($totalData['total_admin_deduct']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                            'color' => 'magenta'
                        ])->style([
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_money_edit_log.total_data.total_activity'))->value(!empty($totalData['total_activity']) ? floatval($totalData['total_activity']) : 0)->valueStyle([
                            'font-size' => '15px',
                            'text-align' => 'center',
                        ])->style([
                            'font-size' => '10px',
            ...
            }, admin_trans('player_money_edit_log.player_info'));
            $grid->column(function (Grid $grid) {
                $grid->column('action', admin_trans('player_money_edit_log.fields.action'))->display(function (
                    ...
                    switch ($val) {
                        ...
                        case PlayerMoneyEditLog::ACTIVITY:
                            return Tag::create(admin_trans('player.wallet.wallet_type.' . $val) . '(' . $this->getActivity($data->activity) . ')')->color('processing');
                        default:
                            return '';
                    }
                })->align('center');
                ...
                $grid->column('after_money',
                    admin_trans('player_money_edit_log.fields.after_money'))->display(function ($val) {
                    return Html::create()->content([
                        $val,
                    ])->style(['color' => '#cd201f']);
                })->sortable()->align('center');
                $grid->column('origin_money',
                    admin_trans('player_money_edit_log.fields.origin_money'))->display(function ($val) {
                    return Html::create()->content([
                        $val,
                    ])->style(['color' => 'green']);
                })->sortable()->align('center');
            }, admin_trans('player_money_edit_log.action_info'));
            ...
            $grid->hideDelete();
            $grid->hideSelection();
            $grid->actions(function (Actions $actions, PlayerMoneyEditLog $data) {
                $actions->hideDel();
            });
            $grid->filter(function (Filter $filter) {
                ...
                $currentLang = \ExAdmin\ui\support\Request::cookie('ex_admin_lang', 'zh-TW');
