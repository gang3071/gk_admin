 ...
 * @group store
 */
class StoreDepositBonusOrderController
{
    protected $model;
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model(), function (Grid $grid) {
            ...
            $grid->model()->where('store_id', $currentAdmin->id)
            ...
            $grid->column('order_no', admin_trans('deposit_bonus_order.fields.order_no'))
                ->display(function ($val, DepositBonusOrder $data) {
                    ...
                    ])->drawer(['addons-webman-controller-StoreDepositBonusOrderController', 'detail'], ['id' => $data->id]);
                })->align('center');
    ...
    public function form(): Form
    {
        return Form::create([], function (Form $form) {
            ...
            // 获取上级代理的活动
            $activities = DepositBonusActivity::where('agent_id', $parentAgent->id)
            ...
            $form->saving(function (Form $form) use ($currentAdmin, $parentAgent) {
                ...
                try {
                    // 店机创建订单：store_id为店机自己，agent_id为上级代理
                    $order = $this->qrcodeService->generateQrcodeOrder(
                        $activityId,
                        $tierId,
                        $player->id,
                        $currentAdmin->id,  // store_id: 当前店机ID
    ...
    public function detail(int $id): Detail
    {
        ...
        $order = DepositBonusOrder::where('id', $id)
            ->where('store_id', $currentAdmin->id)
