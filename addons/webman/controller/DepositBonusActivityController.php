 ...
 * 充值满赠活动管理
 */
class DepositBonusActivityController
{
    protected $model;
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model(), function (Grid $grid) {
            ...
            $grid->autoHeight();
            $grid->bordered(true);
            $grid->model()->with(['tiers'])->orderBy('created_at', 'desc');
            ...
            $grid->column('activity_name', admin_trans('deposit_bonus_activity.fields.activity_name'))
                ->display(function ($val, DepositBonusActivity $data) {
                    ...
                    ])->drawer(['addons-webman-controller-DepositBonusActivityController', 'detail'], ['id' => $data->id]);
                })->align('center');

            $grid->column('store_id', admin_trans('deposit_bonus_activity.fields.store_id'))
                ->display(function ($val) {
                    $channel = Channel::where('department_id', $val)->first();
            ...
            $grid->column('status', admin_trans('deposit_bonus_activity.fields.status'))->switch([
                [1 => ''],
                [0 => '']
            ])->align('center');
    ...
    public function form(): Form
    {
        return Form::create(new $this->model(), function (Form $form) {
            ...
            $form->select('store_id', admin_trans('deposit_bonus_activity.fields.store_id'))
                ->options($this->getChannelOptions())
                ->required()
                ->help(admin_trans('deposit_bonus_activity.help.store_id'));
            ...
            $form->saving(function (Form $form) {
                try {
                    ...
                    if (!$form->isEdit()) {
                        ...
                        $activity = DepositBonusActivity::find($id);
                        if (!$activity) {
                            Db::rollBack();
                    ...
                    $activity->store_id = $form->input('store_id');
    ...
    {
        $data = DepositBonusActivity::with(['tiers'])->find($id);
        return Detail::create($data, function (Detail $detail) use ($data) {
            ...
            $detail->item('store_id', admin_trans('deposit_bonus_activity.fields.store_id'))
                ->display(function ($val) {
                    $channel = Channel::where('department_id', $val)->first();
                    return $channel ? $channel->name : '-';
                });
