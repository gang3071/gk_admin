 ...
 * @group channel
 */
class ChannelNationalPromoterController
{
    protected $model;
    ...
    {
        return Grid::create(new NationalLevel(), function (Grid $grid) {
            for ($i = 5; $i >= 1; $i--) {
                if (!NationalLevel::query()->where('level', $i)->where('department_id', Admin::user()->department_id)->first()) {
                    $level = NationalLevel::query()->create([
                        'level' => $i,
                        'sort' => $i,
                        'department_id' => Admin::user()->department_id,
                        'name' => admin_trans('national_promoter.level_prefix').$i,
                    ...
                    for ($j = 5; $j >= 1; $j--) {
                        if($i == 1 && $j == 1){
                            ...
                            $amount = (2000 * $j) + (10000 * $i);
                        }
                        LevelList::query()->create([
                            'level_id' => $level->id,
                            'department_id' => Admin::user()->department_id,
                            'level' => $j,
                            'must_chip_amount' => $amount,
                            'created_at' => Carbon::now(),
                            'updated_at' => Carbon::now(),
                        ]);
                    }
            ...
            $grid->model()->where('department_id',Admin::user()->department_id)->select('sort', 'name', 'id')->orderBy('sort', 'desc');
            $grid->bordered(true);
            $grid->autoHeight();
            $grid->column('sort', admin_trans('national_promoter.level_list.sort'))->fixed(true)->align('center');
            $grid->column('name', admin_trans('national_promoter.level_list.name'))->editable(
                Editable::text()
    ...
    public function levelDetail($level_id): Grid
    {
        return Grid::create(new LevelList(), function (Grid $grid) use ($level_id) {
            $grid->model()->where('department_id',Admin::user()->department_id)->where('level_id', $level_id)->orderBy('level');
            $grid->autoHeight();
            $grid->bordered(true);
            $grid->column('level', admin_trans('national_promoter.level_list.level'))->display(function (
                $value,
    ...
    {
        $min_level = LevelList::where('department_id', Admin::user()->department_id)->orderBy('level_id', 'desc')->orderBy('level')->first();
        return Form::create(new LevelList(), function (Form $form) use ($min_level) {
            ...
            if ($form->input('id') == $min_level->id) {
                $form->number('must_chip_amount', admin_trans('national_promoter.level_list.must_chip_amount'))
                    ->style(['width' => '100%'])->min(0)->disabled(true)
                    ->required();
            } else {
                $form->number('must_chip_amount', admin_trans('national_promoter.level_list.must_chip_amount'))
            ...
            $form->saving(function (Form $form) {
                ...
                try {
                    ...
                    $subLevelInfo = LevelList::query()->where('department_id', Admin::user()->department_id)->where('level', $level)->where('level_id', $level_id)->first();
                    /** @var LevelList $partLevelInfo */
                    $partLevelInfo = LevelList::query()->where('department_id', Admin::user()->department_id)->where('level', $part_level)->where('level_id',
                        $part_level_id)->first();
    ...
    public function record($id = 0): Grid
    {
        return Grid::create(new PlayerDeliveryRecord(), function (Grid $grid) use ($id) {
            ...
            $grid->model()->with(['player', 'machine'])->when(!empty($id), function ($query) use ($id) {
            ...
            //只展示三个类型的数据
            $grid->model()->whereIn('type',[PlayerDeliveryRecord::TYPE_NATIONAL_INVITE,PlayerDeliveryRecord::TYPE_RECHARGE_REWARD,PlayerDeliveryRecord::TYPE_DAMAGE_REBATE]);
            ...
            $layout->row(function (Row $row) use ($totalData,$totalPoint) {
                ...
                $row->column(
                    Card::create([
                        Row::create()->column(Statistic::create()->title(admin_trans('player_recharge_record.total_data.total_notional_point'))->value(!empty($totalPoint) ?  round($totalPoint,2) : 0)->style([
                            'font-size' => '15px',
                            'text-align' => 'center'
                        ])),
                    ])->bodyStyle([
    ...
    public function NationalProfitRecordDetail($id, string $date = '', string $active_key = '1', array $data = []): Card
    {
        $tabs = Tabs::create($active_key)
            ...
            ->pane(
                ...
                , $this->playerRechargeRecord($id, $date), '2')
            ->pane(
                Html::create()->content([
