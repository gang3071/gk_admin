 ...
 * @group channel
 */
class ChannelPlayerPromoterController
{
    protected $model;
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model, function (Grid $grid) {
            ...
            $grid->autoHeight();
            $grid->bordered(true);
            $content = SystemSetting::where('department_id', Admin::user()->department_id)->where('feature',
                'settlement_date')->value('content');
            ...
            if (empty($content)) {
                ...
                foreach ($contentArr as $item) {
                    $settlementDateText .= $item . admin_trans('player_promoter.date') . ', ';
                }
                $settlementDateText = rtrim($settlementDateText, ', ');
                $settlementDateText .= ')';
            }
            $dropdown = Dropdown::create(
                Button::create(
                    [
                        admin_trans('player_promoter.settlement_date'),
            ...
            for ($i = 1; $i <= 31; $i++) {
                ...
                if (in_array($i, $contentArr)) {
                    $style = ['background-color' => 'orange', 'border-radius' => '6px'];
                }
                $dropdown->item($i . admin_trans('player_promoter.date'))->style($style)->ajax([
                    $this,
                    'setSettlementDate'
                ], ['key' => $i])->gridRefresh();
            }
            $grid->tools([
                ...
                Admin::check(ChannelPlayerPromoterController::class, 'setSettlementDate', 'POST') ? $dropdown : '',
                Button::create(admin_trans('player_promoter.bath_settlement'))
                    ->danger()
                    ->confirm(admin_trans('player_promoter.profit_settlement_confirm'), [$this, 'profitSettlement'])
            ]);
            ...
            $grid->column('name', admin_trans('player_promoter.fields.name'))->display(function (
                ...
                Editable::text()
                    ->showCount()
                    ->rule(['max:200' => admin_trans('player_promoter.name_max_length')])
            ...
            $grid->column('ratio', admin_trans('player_promoter.fields.ratio'))->display(function (
                $val,
                PlayerPromoter $data
            ) use ($grid) {
                $form = Form::create();
                ...
                $form->number('ratio')
                    ->min(1)
                    ->value($val)
                    ->max(isset($data->parent_promoter) ? (int)$data->parent_promoter->ratio : 100)
                    ->precision(2)
                    ->controls(false)
                    ->addonAfter('%')
                    ->help(admin_trans('player_promoter.promoter_max_ratio', null,
                        ['{max_ratio}' => (isset($data->parent_promoter) ? $data->parent_promoter->ratio : 100)]));
                $form->method('PUT');
            ...
            $grid->column('can_settlement_amount',
                ...
                PlayerPromoter $data
                    ...
                    //线下站应结算使用实时结算处理
                if($data->channel->is_offline == 1){
                    //判断用户是否最上级
                    $top = $data->recommend_id == 0;
                    //顶级展示中级营收 和营收之后的比例拆账
                    if(!$top){
                        $amount = PlayerPromoter::query()->withoutGlobalScopes()->where('recommend_id',$data->player_id)->selectRaw("SUM(total_amount*(ratio-$data->ratio))/100 as total_amount")->value('total_amount');
                    }else{
                        $totalPoint = $data->children_total_amount;
                        $amount = bcmul($totalPoint,$data->ratio/100,2);
                    }
                }else{
            ...
            $grid->actions(function (Actions $actions, PlayerPromoter $data) {
                ...
                $dropdown->item(admin_trans('player_promoter.settlement'), 'fas fa-handshake')
                    ->confirm(admin_trans('player_promoter.settlement_confirm', null, [
                        '{uuid}' => $data->player->uuid,
                        '{amount}' => bcadd($data->profit_amount, $data->adjust_amount)
                    ]), [$this, 'settlement'], ['id' => $data->player_id])
                    ->gridRefresh();
     ...
     * @return mixed
     */
    public function pushMessage($departmentId, $userId, $userName)
    {
        Client::send('profit_settlement',
            ['department_id' => $departmentId, 'user_id' => $userId, 'user_name' => $userName]);
     ...
     * @return Msg
     */
    public function setSettlementDate(): Msg
    {
        $data = Request::input();
        $key = $data['key'] ?? 0;
        $content = '';
        $num = SystemSetting::where('department_id', Admin::user()->department_id)->where('feature',
            'settlement_date')->value('content');
        if ($num) {
            $numArr = explode(',', $num);
            if (in_array($key, $numArr)) {
                $k = array_search($key, $numArr);
                if ($k !== false) {
                    unset($numArr[$k]);
                }
            } else {
        ...
        if ($numArr) {
            sort($numArr);
            $content = implode(',', $numArr);
        }
        if (SystemSetting::updateOrCreate(
            ['department_id' => Admin::user()->department_id, 'feature' => 'settlement_date'],
            ['content' => $content, 'status' => 1]
        )) {
            return message_success(admin_trans('player_promoter.action_success'));
        }
        return message_error(admin_trans('player_promoter.action_error'));
    }
     ...
     * @return Msg
     */
    public function settlementOffline($id): Msg
    {
        /** @var Channel $channel */
        $channel = Channel::where('department_id',
        ...
        $playerPromoter = $this->model::where('player_id', $id)->first();
        ...
        //实时结算数据
        OfflineProfitSettlementServices::doProfitSettlement($id);

        $profitAmount = $this->promoterProfitRecord::where('status', PromoterProfitRecord::STATUS_UNCOMPLETED)
            ->where('promoter_player_id', $id)
            ->first([
        ...
        if (!isset($playerPromoter['profit_amount'])) {
            return message_error(admin_trans('player_promoter.profit_amount_not_found'));
        }
        DB::beginTransaction();
        try {
            $settlement = $profitAmount['total_player_profit_amount']??0;
            ...
            $promoterProfitSettlementRecord->user_id = Admin::id() ?? 0;
            $promoterProfitSettlementRecord->user_name = !empty(Admin::user()) ? Admin::user()->toArray()['username'] : '';
            $promoterProfitSettlementRecord->actual_amount = $settlement;
            $promoterProfitSettlementRecord->save();
            // 更新结算报表
            $this->promoterProfitRecord::where('status', PromoterProfitRecord::STATUS_UNCOMPLETED)
                ->where('promoter_player_id', $id)
                ->update([
            ...
            $playerPromoter->children_total_amount = 0;
            // 更新数据
            $playerPromoter->team_profit_amount = $promoterProfitSettlementRecord->total_profit_amount;
            $playerPromoter->last_profit_amount = $settlement;
            $playerPromoter->settlement_amount = bcadd($playerPromoter->settlement_amount,$settlement,2);
            $playerPromoter->team_settlement_amount = bcadd($playerPromoter->team_settlement_amount, $settlement, 2);
     ...
     * @auth true
     * @param $id
     * @param bool $is_settlement
     * @return Msg
     */
    public function settlement($id, bool $is_settlement = true): Msg
    {
        /** @var Channel $channel */
        $channel = Channel::where('department_id',
            Admin::user()->department_id)->whereNull('deleted_at')->first();
        ...
        try {
            ...
            $promoterProfitSettlementRecord->type = $is_settlement ? PromoterProfitSettlementRecord::TYPE_SETTLEMENT : PromoterProfitSettlementRecord::TYPE_CLEAR;
            $promoterProfitSettlementRecord->tradeno = createOrderNo();
            $promoterProfitSettlementRecord->user_id = Admin::id() ?? 0;
            $promoterProfitSettlementRecord->user_name = !empty(Admin::user()) ? Admin::user()->toArray()['username'] : '';
            $settlement = $amount = bcsub(bcadd($promoterProfitSettlementRecord->total_profit_amount,
                $promoterProfitSettlementRecord->adjust_amount, 2), $promoterProfitSettlementRecord->total_commission_amount, 2);
            if ($amount > 0) {
    ...
    public function settlementDetail($id): Detail
    {
        ...
        return Detail::create($settlementRecord, function (Detail $detail) {
            ...
            $detail->item('total_commission_amount',
                admin_trans('promoter_profit_settlement_record.fields.total_commission_amount'));
    ...
    public function profitSettlement(): Notification
    {
        ...
        try {
            /** @var Channel $channel */
            $channel = Channel::where('department_id', $departmentId)
                ->whereNull('deleted_at')
                ->first();
            if ($channel->status == 0) {
                throw new Exception(admin_trans('promoter_profit_settlement_record.channel_closed'));
            }
            if ($channel->promotion_status == 0) {
                throw new Exception(admin_trans('promoter_profit_settlement_record.channel_promotion_closed'));
            }
            $list = PlayerPromoter::whereHas('player', function ($query) {
                ...
                ->get();
            if (empty($list)) {
                throw new Exception(admin_trans('promoter_profit_settlement_record.channel_settlement_promoter_null'));
            }
        } catch (Exception $e) {
            return notification_error(admin_trans('admin.error'), $e->getMessage());
        }
        /** @var PlayerPromoter $playerPromoter */
        foreach ($list as $playerPromoter) {
            try {
    ...
    {
        $children = Player::query()->whereIn('recommend_id', $playerId)->whereNull('deleted_at')->pluck('id')->toArray();
        if(empty($children)){
            return [];
        }
        $totalModel = PlayerDeliveryRecord::query()->whereIn('player_id', $children)->when(!empty($time),function($query)use($time){
