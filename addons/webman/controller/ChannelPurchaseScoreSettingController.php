...
use addons\webman\model\PurchaseScoreSetting;
 ...
 * @group channel
 */
class ChannelPurchaseScoreSettingController
{
    protected $model;

    public function __construct()
    {
        $this->model = PurchaseScoreSetting::class;
    }
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model(), function (Grid $grid) {
            $grid->title(admin_trans('purchase_score_setting.title'));
            $grid->model()->where('store_admin_id', Admin::user()->id)->orderBy('id', 'desc');
            $grid->autoHeight();
            $grid->bordered(true);
    ...
    public function form(): Form
    {
        return Form::create(new $this->model(), function (Form $form) {
            ...
            $form->saving(function (Form $form) {
                ...
                $form->input('store_admin_id', $storeAdminId);

                // 检查是否已存在配置（编辑时排除当前记录）
                $exists = $this->model::query()->where('store_admin_id', $storeAdminId);
