 ...
 * 活动管理
 */
class ChannelPlayerActivityRecordController
{
    protected $model;
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model, function (Grid $grid) {
            ...
            $grid->column('status', admin_trans('player_activity_record.fields.status'))->display(function (
                $val,
                PlayerActivityRecord $data
            ) {
                switch ($data->status) {
            ...
            $grid->filter(function (Filter $filter) {
                ...
                $filter->form()->dateRange('created_at_start', 'created_at_end', '')->placeholder([
                    admin_trans('player_activity_record.created_at_start'),
                    admin_trans('player_activity_record.created_at_end')
                ]);
            });
            $grid->hideDelete();
            $grid->hideTrashed();
    ...
    public function playerActivityPhaseRecord($id): Grid
    {
        return Grid::create(new $this->playerActivityPhaseRecordModel, function (Grid $grid) use ($id) {
            ...
            $grid->column('created_at',
                admin_trans('player_activity_phase_record.fields.created_at'))->align('center');
            $grid->hideDelete();
            $grid->hideTrashed();
    ...
    protected function exmaineGrid(Grid $grid, string $lang): void
    {
        ...
        $grid->column(function (Grid $grid) {
            ...
            $grid->column('odds', admin_trans('player_game_log.fields.odds'))->display(function (
                $val,
                PlayerActivityPhaseRecord $data
            ) {
        ...
        $grid->column('player_score',
            admin_trans('player_activity_phase_record.fields.player_score'))->align('center');
        $grid->expandFilter();
    }
    ...
    public function examine(): Grid
    {
        return Grid::create(new $this->playerActivityPhaseRecordModel(), function (Grid $grid) {
            ...
            ])->where('status', PlayerActivityPhaseRecord::STATUS_RECEIVED)->orderBy('created_at', 'desc');
            $this->exmaineGrid($grid, $lang);
            $grid->column('status', admin_trans('player_activity_phase_record.fields.status'))->display(function (
                $val
            ) {
            ...
            $dropdown->item(admin_trans('player_activity_phase_record.btn.examine_reject'),
            ...
            $grid->actions(function (Actions $actions, PlayerActivityPhaseRecord $data) {
                ...
                $dropdown = Dropdown::create(
                    Button::create([
                        admin_trans('player_activity_phase_record.btn.action'),
                        Icon::create('DownOutlined')->style(['marginRight' => '5px'])
                    ]))->trigger(['click']);

                $dropdown->item(admin_trans('player_activity_phase_record.btn.examine_pass'),
                    'SafetyCertificateOutlined')
                    ->confirm(admin_trans('player_activity_phase_record.btn.examine_pass_confirm'), [$this, 'pass'],
                        ['id' => $data->id])
                    ->gridRefresh();
    ...
    public function bathPass(): Msg
    {
        ...
        if (!empty($selected)) {
            $playerActivityPhaseRecords = $this->playerActivityPhaseRecordModel::with('player.machine_wallet')->whereIn('id',
                $selected)->orderBy('id', 'desc')->get();
            ...
            try {
                foreach ($playerActivityPhaseRecords as $playerActivityPhaseRecord) {
                    ...
                    $notice->content = str_replace(
                        ['{machine_code}', '{bonus}'],
                        [$playerActivityPhaseRecord->machine->code, $playerActivityPhaseRecord->bonus],
                        admin_trans('player_activity_record.notice.activity_pass_content_with_machine')
                    );
                    $notice->save();
                }
    ...
    public function bathReject(): Form
    {
        return Form::create(new $this->model(), function (Form $form) {
            ...
            $form->saving(function (Form $form) {
                ...
                $playerActivityPhaseRecords = $this->playerActivityPhaseRecordModel::whereIn('id',
                    $selected)->get();
                ...
                foreach ($playerActivityPhaseRecords as $playerActivityPhaseRecord) {
                    ...
                    $notice->content = str_replace(
                        '{reason}',
                        $form->input('reject_reason') ?? '',
                        admin_trans('player_activity_record.notice.activity_reject_content')
                    );
                    $notice->save();
                }
        ...
        return Form::create(new $this->playerActivityPhaseRecordModel(), function (Form $form) use ($id) {
            ...
            $form->saving(function (Form $form) use ($id) {
                ...
                try {
                    ...
                    $notice->content = str_replace(
                        '{reason}',
                        $playerActivityPhaseRecord->reject_reason,
                        admin_trans('player_activity_record.notice.activity_reject_content')
                    );
                    $notice->save();
                } catch (\Exception $e) {
    ...
    public function pass($id): Msg
    {
        ...
        try {
            ...
            $notice->content = str_replace(
                '{bonus}',
                $playerActivityPhaseRecord->bonus,
                admin_trans('player_activity_record.notice.activity_pass_content')
            );
            $notice->save();
            DB::commit();
        } catch (\Exception $e) {
