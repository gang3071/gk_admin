 ...
 * 机台保留日志
 */
class MachineKeepingLogController
{
    protected $model;
    ...
    public function index(): Grid
    {
        return Grid::create(new $this->model, function (Grid $grid) {
            ...
            $grid->column(function (Grid $grid) {
                ...
                $grid->column('player.phone', admin_trans('player.fields.phone'))->display(function ($val, MachineKeepingLog $data) {
                ...
                $grid->column('player.channel.name', admin_trans('player.fields.department_id'))->display(function ($val, MachineKeepingLog $data) {
                    if(isset($data->player)){
            ...
            $grid->column(function (Grid $grid) {
                $grid->column('machine.name', admin_trans('machine.fields.name'))->display(function ($val, MachineKeepingLog $data) {
                    if ($data->machine) {
                ...
                $grid->column('machine.code', admin_trans('machine.fields.code'))->display(function ($val, MachineKeepingLog $data) {
                    return $data->machine->code;
                })->align('center');
            }, admin_trans('machine_keeping_log.machine_info'));

            $grid->column('user.username', admin_trans('admin.admin_user'))->display(function ($val, MachineKeepingLog $data) {
                if ($data->user) {
                    $image = Image::create()
                        ->width(30)
                        ->height(30)
            ...
            $grid->column('keep_seconds', admin_trans('machine_keeping_log.fields.keep_seconds'))->display(function ($val, MachineKeepingLog $data) {
            ...
            $grid->filter(function (Filter $filter) {
                ...
                $filter->form()->dateTimeRange('created_at_start', 'created_at_end', '')->placeholder([admin_trans('machine_keeping_log.created_at_start'), admin_trans('machine_keeping_log.created_at_end')]);
            });
            $grid->expandFilter();
        });
    }
