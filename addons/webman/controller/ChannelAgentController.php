...
class ChannelAgentController
{
    ...
    public function machineList(): Grid
    {
        ...
        return Grid::create($list, function (Grid $grid) use ($admin, $total, $list, $playerOptions, $channelVipLevels) {
            ...
            $grid->column('recharge_amount', admin_trans('channel_agent.fields.recharge_amount'))->display(function ($value, $data) {
                // 累计开分需要扣除投钞金额（因为开分字段已包含投钞）
                $pureRecharge = $data['pure_recharge_amount'] ?? 0;
                return number_format(floatval($pureRecharge), 2);
            })->sortable()->width('100px')->align('center');
    ...
    {
        return Grid::create(new $this->playerDeliveryRecord(), function (Grid $grid) {
