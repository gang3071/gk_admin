...
class ChannelAgentController
{
    ...
    public function machineList(): Grid
    {
        ...
        return Grid::create($list, function (Grid $grid) use ($admin, $total, $list) {
            ...
            // 批量创建玩家按钮
            $grid->tools(
                Button::create(admin_trans('offline_channel.batch_generate_players'))
                    ->icon(Icon::create('fas fa-users-cog'))
    ...
    public function batchGenerateForm(): Form
    {
        ...
        return Form::create([], function (Form $form) {
            ...
            $form->push(Html::markdown('><font size=2 color="#1890ff">' . admin_trans('offline_channel.tip_batch_agent') . '</font>'));
            ...
            $form->row(function (Form $form) {
                $form->column(function (Form $form) {
                    ...
                    $form->text('phone_prefix', admin_trans('offline_channel.account_prefix'))
                        ->default('P')
                        ->maxlength(10)
                        ->help(admin_trans('offline_channel.help_account_prefix'))
                        ->required();
                })->span(12);
                ...
                $form->column(function (Form $form) {
                    ...
                    $form->number('phone_start_number', admin_trans('offline_channel.account_start_number'))
                        ...
                        ->precision(0)
                        ->style(['width' => '100%'])
                        ->help(admin_trans('offline_channel.help_account_start_number'))
                        ->required();
                })->span(12);
            })->gutter(16);
            ...
            $form->row(function (Form $form) {
                $form->column(function (Form $form) {
                    ...
                    $form->text('name_prefix', admin_trans('offline_channel.nickname_prefix'))
                        ...
                        ->maxlength(10)
                        ->help(admin_trans('offline_channel.help_nickname_prefix'))
                        ->required();
                })->span(12);
                ...
                $form->column(function (Form $form) {
                    ...
                    $form->number('name_start_number', admin_trans('offline_channel.nickname_start_number'))
                        ...
                        ->precision(0)
                        ->style(['width' => '100%'])
                        ->help(admin_trans('offline_channel.help_nickname_start_number'))
                        ->required();
                })->span(12);
            })->gutter(16);
            ...
            $form->radio('avatar_type', admin_trans('offline_channel.avatar_source'))
                ...
                ->when(['default'], function (Form $form) {
                    ...
                    $form->select('default_avatar', admin_trans('offline_channel.select_default_avatar'))
                        ...
                        ->help(admin_trans('offline_channel.help_all_players_avatar'))
                        ->required();
                })
                ->when(['upload'], function (Form $form) {
                    $form->image('custom_avatar', admin_trans('offline_channel.upload_avatar'))
                        ->help(admin_trans('offline_channel.help_upload_avatar_batch'))
                        ->required();
                })
                ->required();
            ...
            $form->password('password', admin_trans('offline_channel.unified_password'))
                ...
                ->help(admin_trans('offline_channel.help_all_accounts_password'))
                ->required();
    ...
    private function batchGeneratePlayersSaving(Form $form)
    {
        ...
        // 新架构：获取代理的第一个下级店家
        // TODO: 可能需要让用户选择归属哪个店家
        $firstStore = null;
        if ($admin->type === \addons\webman\model\AdminUser::TYPE_AGENT) {
            $firstStore = $admin->childStores()
                ->where('type', \addons\webman\model\AdminUser::TYPE_STORE)
                ->first();
            if (!$firstStore) {
                return message_error('代理账号下没有店家，无法创建玩家账号');
            }
        ...
        try {
            ...
            for ($i = 0; $i < $generateCount; $i++) {
                ...
                if (!empty($existingPlayer)) {
                    $failedAccounts[] = $phone . '(' . admin_trans('offline_channel.account_exists') . ')';
                    continue;
                }
                ...
                // 新架构：关联到店家和代理
                $player->store_admin_id = $firstStore->id; // 归属店家
                if ($admin->type === \addons\webman\model\AdminUser::TYPE_AGENT) {
                    $player->agent_admin_id = $admin->id; // 归属代理
                }
            ...
            $message = admin_trans('offline_channel.success_generate_players', null, ['{count}' => $successCount]);
            if (count($failedAccounts) > 0) {
                $message .= admin_trans('offline_channel.failed_generate_count', null, ['{count}' => count($failedAccounts)]) . implode(', ', $failedAccounts);
            }
            ...
            Db::rollBack();
            return message_error(admin_trans('offline_channel.error_batch_generate_failed', null, ['{error}' => $e->getMessage()]));
        }
    }
