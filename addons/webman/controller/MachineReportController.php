    ...
    public function index(): Grid
    {
        ...
        if (!empty($exAdminFilter)) {
            ...
            if (!empty($exAdminFilter['department_id'])) {
                $machineReport->where('department_id', $exAdminFilter['department_id']);
            }
            if (!empty($exAdminFilter['is_test'])) {    //筛选测试数据
                $machineReport->where('is_test', '=', $exAdminFilter['is_test']);
            }
        ...
        return Grid::create($list, function (Grid $grid) use ($total, $list, $exAdminFilter) {
            ...
            $grid->filter(function (Filter $filter) {
                ...
                $filter->in()->cascaderSingle('machine.cate_id')
                    ...
                    ->multiple();
                $filter->select('is_test')->placeholder(admin_trans('machine_report.fields.data_type')) //筛选条件选择
                    ->showSearch()
                    ->dropdownMatchSelectWidth()
                    ->style(['width' => '200px'])
                    ->options([
                        0 => admin_trans('machine_report.is_test0'),
                        1 => admin_trans('machine_report.is_test1'),
                    ]);
            });
            ...
            $grid->actions(function (Actions $action, $data) use ($grid) {
                ...
                $action->prepend([
                    Button::create(admin_trans('machine_report.details'))
                        ...
                        ->type('primary')
                        ->size('small')
                        ->modal('ex-admin/addons-webman-controller-PlayerGameLogController/list', [
