 ...
 * @group channel
 */
class ChannelPlayerLotteryRecordController
{
    protected $model;
        ...
        return Grid::create(new $this->model(), function (Grid $grid) {
            ...
            $layout->row(function (Row $row) use ($requestFilter) {
                ...
                $row->column(admin_view(plugin()->webman->getPath() . '/views/total_info.vue')->attrs([
                    ...
                    'type' => 'ChannelPlayerLotteryRecord',
                ]));
            ...
            $grid->column('player_phone', admin_trans('player_lottery_record.fields.player_phone'))->display(function ($val, PlayerLotteryRecord $data) {
            ...
            $grid->column('machine_code',
                admin_trans('player_lottery_record.fields.machine_code'))->sortable()->align('center')->copy();
            ...
            $grid->column('amount', admin_trans('player_lottery_record.fields.amount'))->display(function ($val, PlayerLotteryRecord $data) {
                return Html::create()->content([
                    Html::div()->content($val),
            ...
            $grid->column('lottery_rate', admin_trans('player_lottery_record.fields.lottery_rate'))->display(function ($val) {
                return Html::create()->content([
                    Html::div()->content($val . '%')
                ]);
            })->sortable()->align('center');
            $grid->column('cate_rate', admin_trans('player_lottery_record.fields.cate_rate'))->align('center');
            $grid->column('reject_reason', admin_trans('player_lottery_record.fields.reject_reason'))->display(function ($value) {
                return Str::of($value)->limit(20, ' (...)');
            })->tip()->width('150px')->align('center');
            $grid->column('user_name', admin_trans('player_lottery_record.fields.user_name'))->display(function ($val, PlayerLotteryRecord $data) {
                return Html::create()->content([
    ...
    public function auditList(): Grid
    {
        return Grid::create(new $this->model(), function (Grid $grid) {
            ...
            $grid->autoHeight();
            $requestFilter = Request::input('ex_admin_filter', []);
            $grid->model()->with(['channel'])->where('lottery_type', Lottery::LOTTERY_TYPE_FIXED)->where('status', PlayerLotteryRecord::STATUS_UNREVIEWED)->orderBy('created_at', 'desc');
            if (!empty($requestFilter['created_at_start'])) {
                $grid->model()->where('created_at', '>=', $requestFilter['created_at_start']);
            }
            if (!empty($requestFilter['created_at_end'])) {
            ...
            $grid->column('machine_name', admin_trans('player_lottery_record.fields.machine_name'))->display(function ($val, PlayerLotteryRecord $data) {
                if ($data->machine) {
                    return Tag::create($val)->color('orange')->style(['cursor' => 'pointer'])->modal([PlayerDeliveryRecordController::class, 'machineInfo'], ['data' => $data->machine->toArray()])->width('60%')->title($data->machine->code . ' ' . $data->machine->name);
                }
                return '';
            })
                ->align('center');
            $grid->column('machine_code', admin_trans('player_lottery_record.fields.machine_code'))->align('center')->copy();
            $grid->column('odds', admin_trans('player_lottery_record.fields.odds'))->align('center');
            $grid->column('lottery_name', admin_trans('player_lottery_record.fields.lottery_name'))->display(function ($value, PlayerLotteryRecord $data) {
            ...
            $grid->column('lottery_pool_amount', admin_trans('player_lottery_record.fields.lottery_pool_amount'))->align('center');
            $grid->column('lottery_rate', admin_trans('player_lottery_record.fields.lottery_rate'))->display(function ($val) {
                return Html::create()->content([
                    Html::div()->content($val . '%')
                ]);
            ...
            $grid->actions(function (Actions $actions, PlayerLotteryRecord $data) {
                ...
                $dropdown = Dropdown::create(
                    Button::create([
                        admin_trans('player_lottery_record.btn.action'), Icon::create('DownOutlined')->style(['marginRight' => '5px'])
                    ]))->trigger(['click']);

                $dropdown->item(admin_trans('player_lottery_record.btn.examine_pass'), 'SafetyCertificateOutlined')
                    ->confirm(admin_trans('player_lottery_record.btn.examine_pass_confirm'), [$this, 'pass'], ['id' => $data->id])->gridRefresh();
