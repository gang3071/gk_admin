...
if (!function_exists('checkMachineCrash')) {
    /**
     * 检查设备是否爆机
     *
    ...
    function checkMachineCrash(Player $player): array
    {
        ...
        try {
            ...
            // Redis 故障时降级到数据库查询
            \support\Log::error('checkMachineCrash: Redis get failed', [
                'player_id' => $player->id,
                'error' => $e->getMessage(),
            ]);
        }
        ...
        try {
            ...
            // 缓存写入失败不影响业务
            \support\Log::error('checkMachineCrash: Redis setex failed', [
                'player_id' => $player->id,
                'error' => $e->getMessage(),
            ]);
        }
...
if (!function_exists('notifyMachineCrash')) {
    /**
     * 通知设备爆机
     * @param Player $player 玩家对象
...
if (!function_exists('calculateAllowedWithdrawAmount')) {
    /**
     * 计算爆机状态下允许的最大洗分金额
...
if (!function_exists('checkAndNotifyCrashUnlock')) {
    /**
     * 检查并通知爆机解锁
     * 用于洗分后检查是否已解锁爆机状态
     * @param Player $player 玩家对象
